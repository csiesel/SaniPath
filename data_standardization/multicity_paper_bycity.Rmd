...
df.lab <- df.lab %>% left_join(., meta_dply[c("id", "city", "citylabel")], by = c("dply_num" = "id"))
...
meta_dply %>% arrange(id) %>% select("country", "city", "year", "citylabel") %>% 
...
meta_neighb %>% 
        ...
        arrange(id) %>% 
        select(city, country, year, neighb_UID, neighborhood) %>% 
...
df <- as.data.frame.matrix(table(df.col$col_sample_type, df.col$city))
...
df.h %>% group_by(city) %>% summarize("nr" = n()) %>% spread(city, nr) %>% 
        as.data.frame() %>% mutate("Form" = "Household") -> nr.h
df.c %>% group_by(city) %>% summarize("nr" = n()) %>% spread(city, nr) %>% 
        as.data.frame() %>%mutate("Form" = "Community") -> nr.c
df.s %>% group_by(city) %>% summarize("nr" = n()) %>% spread(city, nr) %>% 
        as.data.frame() %>%mutate("Form" = "School") -> nr.s
df.col %>% group_by(city) %>% summarize("nr" = n()) %>% spread(city, nr) %>% 
        as.data.frame() %>%mutate("Form" = "Sample") -> nr.col
df.lab %>% group_by(city) %>% summarize("nr" = n()) %>% spread(city, nr) %>% 
...
df.h %>% group_by(city) %>% summarize("nr" = n()) %>% # use n() here because only 1 person is interviewed per survey
        spread(city, nr) %>% mutate("Form" = "Household") -> nr.h
df.c %>% group_by(city) %>% summarize("nr" = sum(c_participant_num)) %>% 
        spread(city, nr) %>% mutate("Form" = "Community") -> nr.c
df.s %>% group_by(city) %>% summarize("nr" = sum(s_participant_num)) %>% 
        spread(city, nr) %>% mutate("Form" = "School") -> nr.s
...
df <- df.ecdata %>% select(city, mf) %>% table() %>% as.data.frame.matrix()
...
# by city
df.lab %>% filter(lab_analysis == 2) %>% group_by(city) %>%
        ...
        select(city, n, valid, invalid, `percent valid`)%>%
...
# by city and sample
df.lab %>% filter(lab_analysis == 2) %>% group_by(city, lab_sample_type) %>%
        summarise("n" = sum(!is.na(lab_1_ecoli_reading_membrane)) +
                          sum(!is.na(lab_2_ecoli_reading_membrane)) + 
                          sum(!is.na(lab_3_ecoli_reading_membrane)), 
                  "valid" = sum(lab_1_ecoli_reading_membrane == 3, na.rm = T) +
        ...
        mutate(`percent valid` = cell_spec(`percent valid`, "html", 
                                           color = ifelse(`percent valid` < colorcutoff, "red", "black")) ) %>%
        select(city, "sample type" = sample_type_name, n, valid, invalid, `percent valid`) %>%
        kable(., format = "html",  escape = F, caption = "Valid Membrane Filtration Lab Results by city and Sample Type") %>% 
        kable_styling(bootstrap_options = c("hover", "bordered","condensed"), full_width = F, position = "left")
...
df %>% group_by(city) %>%
        ggplot(., aes(x=reorder(city, desc(city)), y=percentage, fill=city))+ #reverse order for horizontal graph
...
```{r echo=FALSE}
df.ecdata %>% select(city, ec_conc, neighb_UID) %>% 
        group_by(city, neighb_UID) %>% 
        summarise(omitted = sum(is.na(ec_conc)),
                  n = n(),
...
df %>% filter(percentage != 0) %>%
        ggplot(., aes(x=factor(neighb_UID), y=percentage, fill=city))+
        geom_bar(stat = "identity") +
        labs(fill = "", 
             title = "Percentage of Dilution Error",
             x = "Neighborhood") +
...
```{r echo=FALSE}
df.ecdata %>% select(city, ec_conc, neighb_UID, sample_type_name) %>% 
        group_by(city, neighb_UID, sample_type_name) %>% 
        summarise(omitted = sum(is.na(ec_conc)),
                  n = n(),
                  percentage = round((omitted/n)*100, 2)) -> df
...
df2 %>% ggplot(., aes(x=factor(city), y=log10(ec_conc), fill=city)) +
        stat_boxplot(geom = "errorbar", width = 0.35) +
        geom_boxplot() +
...
df.col %>% group_by(city) %>% count(col_p_type) %>% na.omit() %>% 
        left_join(., meta_produce, by = c("col_p_type" = "produce_id")) %>% select(-col_p_type, -id) %>%
        kable(., caption = "Types of Produce, by city") %>% 
        kable_styling(bootstrap_options = c("hover", "bordered","condensed"), full_width = F, position = "left")
...
cities <- meta_dply$city[!meta_dply$city %in% c("Maputo")]
...
df2 %>% left_join(., latrines, by = c("col_l_toilet_type" = "answer")) %>% 
        ggplot(., aes(x=factor(city), y=log10(ec_conc), fill=factor(city))) +
        stat_boxplot(geom = "errorbar", width = 0.35) +
        geom_boxplot() +
...
df.col %>% select(-(city)) %>% 
        left_join(., df.ecdata, by = c("col_UID" = "UID")) %>%
        group_by(city) %>% count(col_l_toilet_type) %>% na.omit() %>% 
        left_join(., latrines, by = c("col_l_toilet_type" = "answer")) %>% select(-col_l_toilet_type) %>%
...
df2 %>% group_by(city, col_l_toilet_type) %>% summarise("n" = n(), 
                                                  "min ecoli" = round(log10(min(ec_conc, na.rm = T)),2),
                                                  "max ecoli" = round(log10(max(ec_conc, na.rm = T)),2),
                                                  "mean ecoli" = round(log10(mean(ec_conc, na.rm = T)),2),
                                                  "standard deviation" = round(log10(sd(ec_conc, na.rm = T)),2),
        ...
        left_join(., latrines, by = c("col_l_toilet_type" = "answer")) %>% select(-col_l_toilet_type) %>% 
        select(Type, everything()) %>%
        kable(., caption = "E. coli count per Types of Latrines by city") %>% 
        kable_styling(bootstrap_options = c("hover", "bordered","condensed"), full_width = F, position = "left") 

df2 %>% group_by(city, neighb_UID) %>% summarise("n" = n(), 
                                                  "min ecoli" = round(log10(min(ec_conc, na.rm = T)),2),
                                                  "max ecoli" = round(log10(max(ec_conc, na.rm = T)),2),
                                                  "mean ecoli" = round(log10(mean(ec_conc, na.rm = T)),2),
                                                  "standard deviation" = round(log10(sd(ec_conc, na.rm = T)),2),
        ...
        select(city, "Neighborhood" = neighb_UID, everything()) %>%
        kable(., caption = "E. coli count of all latrines by city") %>% 
        kable_styling(bootstrap_options = c("hover", "bordered","condensed"), full_width = F, position = "left") 
...
df2 %>% left_join(., latrines, by = c("col_l_toilet_type" = "answer")) %>% 
        ...
        geom_boxplot() +
        facet_wrap(~city) +
        labs(title = "E. coli count by latrine Type and City",
             fill = "Latrine Type",
             x = "Latrine Type") +
        theme_bw()
...
df %>% group_by(city) %>% summarise("n" = n(), 
                                        "min ecoli" = round(log10(min(ec_conc, na.rm = T)),2),
                                        "max ecoli" = round(log10(max(ec_conc, na.rm = T)),2),
                                        "mean ecoli" = round(log10(mean(ec_conc, na.rm = T)),2),
                                        "standard deviation" = round(log10(sd(ec_conc, na.rm = T)),2),
        ...
        kable(., caption = "Streetfood E. coli count per city") %>% 
        kable_styling(bootstrap_options = c("hover", "bordered","condensed"), full_width = F, position = "left")
        # column_spec(2, border_right = T) 



df %>% group_by(neighb_UID, city) %>% summarise("n" = n(), 
                                        "min ecoli" = round(log10(min(ec_conc, na.rm = T)),2),
                                        "max ecoli" = round(log10(max(ec_conc, na.rm = T)),2),
                                        "mean ecoli" = round(log10(mean(ec_conc, na.rm = T)),2),
                                        "standard deviation" = round(log10(sd(ec_conc, na.rm = T)),2),
...
### Graphs - Differences within Produce Samples
```{r echo=FALSE, warning=FALSE}
df %>% ggplot(., aes(x=factor(city), y=log10(ec_conc), fill=factor(city))) +
        stat_boxplot(geom = "errorbar", width = 0.35) +
        geom_boxplot() +
...
df %>% ggplot(., aes(x=factor(neighb_UID), y=log10(ec_conc), fill=factor(neighb_UID))) +
        stat_boxplot(geom = "errorbar", width = 0.35) +
        geom_boxplot() +
        facet_grid(~city, scales = "free", space = "free_x") +
...
df %>% ggplot(., aes(x=reorder(neighb_UID, desc(neighb_UID)), y=log10(ec_conc), fill=factor(city))) +
        stat_boxplot(geom = "errorbar", width = 0.35) +
        geom_boxplot() +
        labs(title = "Streetfood E. coli count by Neighborhood",
             fill = "Neighborhood",
...
# by deployment
df %>% ggplot(., aes(x=factor(sample_type_name), y=log10(ec_conc), fill=factor(city))) +
        # stat_boxplot(geom = "errorbar", width = 0.35) +
        geom_boxplot() +
...
df33 %>% ggplot(., aes(x=factor(col_sample_type_alt), y=log10(ec_conc), fill=factor(city))) +
        geom_boxplot() +
