...
# adults -----
{
        ...
        for (w in pathways){
                ...
                df.h %>% select(h_UID, neighb_UID, "coding" = var) %>%
                        group_by(h_UID) %>%
                        summarise(a3 = sum(coding==1, na.rm = T),
                                  ...
                                  sum1 = n(),
                                  sum=(some1+never1)) %>%
                        select(h_UID, a3, a2, a1, a0, some1, never1, sum) %>% 
                        data.frame() -> df1
                
                df1$neighb_UID <- 0
                for(i in 1:nrow(df1)){
                        df1$neighb_UID[i] <- df.h$neighb_UID[which(df1$h_UID[i]==df.h$h_UID)]
                }
                
                df.comb <- df1 %>% summarise(a3 = sum(a3),
                                                            a2 = sum(a2),
                                                            a1 = sum(a1),
                                                            a0 = sum(a0),
                                                            some1 = sum(some1),
                ...
                df.comb <- select(df.comb, h_UID, neighb_UID, a3, a2, a1, a0)
