...
df.ecdata %>% group_by(sample_type_name, city) %>% 
        summarise(min = log10(min(ec_conc, na.rm = T)),
                  max = log10(max(ec_conc, na.rm = T)),
                  mean = log10(mean(ec_conc, na.rm = T)),
                  median = log10(median(ec_conc, na.rm = T))) -> df

df.ecdata %>% group_by(col_sample_type_alt, city) %>% 
        summarise(min = log10(min(ec_conc, na.rm = T)),
                  max = log10(max(ec_conc, na.rm = T)),
                  mean = log10(mean(ec_conc, na.rm = T)),
                  median = log10(median(ec_conc, na.rm = T))) -> df2
...
df.ecdata %>%  filter(sample_type == 3) %>% 
        group_by(city) %>% summarise(min = log10(min(ec_conc, na.rm = T)),
                                     max = log10(max(ec_conc, na.rm = T)),
                                     mean = log10(mean(ec_conc, na.rm = T)),
                                     median = log10(median(ec_conc, na.rm = T)))
