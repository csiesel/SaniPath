...
pdf(paste("~/Desktop/SaniPath/Dakar/dakar_ec.pdf",sep=""), width = 13.3, height=7.5)

df.ecdata %>%
  filter(city=="Dakar") %>%
  ggplot(., aes(y=factor(hood, levels=(unique(hood))), x=log10(ec_conc), shape=SES)) +
...
#------------------------------------------
pdf(paste("~/Desktop/SaniPath/kampala_bx.pdf",sep=""), width = 13.3, height=7.5)
df.behav.city %>%
  filter(., city =="Kampala") %>%
